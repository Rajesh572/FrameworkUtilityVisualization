...
import { CreateUpdateReqService } from 'src/app/modules/core/Services/CreateUpdateReq/create-update-req.service';
import { DeleteDataService } from 'src/app/modules/core/Services/deleteData/delete-data.service';
import { FWTermsReadService } from '../../Services/FWTermsRead/fwterms-read.service';
import { PublishFrameworkService } from 'src/app/modules/core/Services/publishFramework/publish-framework.service';
import { DownloadExcelService } from 'src/app/modules/core/Services/DownloadExcel/download-excel.service';
import { SetDefaultService } from 'src/app/modules/core/Services/setDefault/set-default.service';
import { GetStatusService } from '../../Services/getStatus/get-status.service';
import { ProcessStatusService } from '../../Services/ProcessStatus/process-status.service';
import { ProcessIdStorageService } from '../../Services/processId/process-id-storage.service';
...
export class ModalComponent implements OnInit {
  ...
  getProcessStatus() {
    const req = this.getReport.getstatusreport().subscribe((res) => {
      console.log('Response of json', res);
     const items = this.processIdData.getData();
        items.forEach((val) => {
  ...
  sendReq() {
    if (this.data['action'] === 'delete') {
      ...
      const req = this.createupdate.sendingRequest(newFd)
        .subscribe((res) => {
          this.dialogRef.close();
  ...
  publishFramework() {
    ...
    const req = this.fwPublishService.publishFramework(this.fwcode, pid).subscribe((res) => {
      if (res === null || res === undefined) {
        console.log('In response got ', res);
        req.unsubscribe();
        return;
  ...
  downloadExcel() {
    ...
    const req = this.exportExcel.exportExcel(this.fwcode, pid).subscribe(
      (res) => {
        if (res === null || res === undefined) {
          console.log('In response got ', res);
          return;
  ...
  setAllLogs(res, val, action?) {
    ...
    const req = this.errorhandler.sendErrorMessage(this.getTime() + '   ' + msg).subscribe((response) => {
  ...
  readSubTerms(fwcode, catgCode, codeOfItem) {
    const req = this.termRead.readTerms(fwcode, catgCode, codeOfItem).subscribe((res) => {
      let temp = [];
  ...
  checkStatus() {
    const req = this.getStatus.retrieveStatus().subscribe((res) => {
  ...
  updateProcessIdArray(pid) {
    ...
    newData.push({
      'processId': pid, 'startTime': this.getTime(),
      'frameworkcode': this.code ? this.code : this.fwcode
    });
        ...
        if (res.hasOwnProperty(key)) {
          const items =  this.processIdData.getData();
              items.forEach((val, index) => {
                if (!(val['endTime']) && val['processId'] === key) {
                   this.processIdData.setData(index, this.getTime());
                }
              });
        }
      }
